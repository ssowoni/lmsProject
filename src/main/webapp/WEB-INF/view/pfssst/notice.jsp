...
/* 		$("#to_date").change(function() {
		...
		$("#searchvalue").focus().keydown(function (key) { // 로딩완료후 검색창 포커스 & 엔터키 입력 검색
	        if (key.keyCode == 13) {
	            $("#searchBtn").click();
	        }
	    });
		...
		$(document).on("click",".popup",function(){ // ???? 왜 $(function{}) 에 넣었을땐 안되는지 .. ???? 
			...
			var noticeNo = $(this).attr("href");  // tr 태그엔 데이터를 저장할 val이 없기때문에 href로 대체사용 .
			console.log('List Click!!');
			console.log($(this).attr("href"));
			
			fNoticeModal(noticeNo);
...
//alert("지금 현재 페이지를 찍어봅시다. " + currentPage);
		...
		to_date =$("#to_date").val();
		searchselect = $("#searchselect option:selected").val();
	 ...
	 function searchNoticeList(currentPage) {
	 ...
	 function fdetailModal(noticeNo){
		 ...
		 callAjax("/supportD/detailNotice.do", "post", "json", true, param, resultCallback2);
		 //alert("공지사항 상세 조회  22");
	 }
				...
				<li class="contents">
					...
					<table width="100%" cellpadding="5" cellspacing="0" border="1"
                        ...
                        <tr style="border: 0px; border-color: blue">
                           ...
                           <td width="25px" height="25" style="font-size: 100%;">
                           <select id="searchselect" style="height:20px; min-width:60px; max-width : 100px; text-align:center;">		
								<option value="1" selected>제목</option>
								<option value="2">글 내용</option>							
							</select>
							</td>
                           <td width="50" height="25" style="font-size: 100%;">
							<input type="text" style="width: 200px" id="searchvalue" name="searchvalue" placeholder="검색할 제목/내용을 입력하세요."/></td>         
